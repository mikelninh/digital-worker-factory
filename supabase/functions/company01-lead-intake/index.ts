...
Deno.serve(async (req) => {
  ...
  const rpcBody = {
    ...
    p_email: email,
    p_source: clean(body.source || "authority_scorecard", 80),
    p_sector: clean(body?.input?.sector, 40),
    p_agent_stage: clean(body?.input?.agentStage, 40),
    p_readiness_score: Number.isFinite(Number(score.readiness)) ? Number(score.readiness) : null,
    p_authority_risk_score: Number.isFinite(Number(score.risk)) ? Number(score.risk) : null,
    p_consequence_signals: Number.isFinite(Number(score.consequenceSignals)) ? Number(score.consequenceSignals) : null,
    p_urgency: clean(score.urgency, 20) || null,
    p_recommended_pilot: clean(score.recommendedPilot, 120) || null,
