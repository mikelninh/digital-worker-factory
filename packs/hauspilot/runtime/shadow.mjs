...
export const OUTPUT_SCHEMA = {
  ...
  properties: {
    ...
    proposed_action: {
      ...
      required: ['type', 'description', 'external', 'consequential'],
      additionalProperties: false
    },
  ...
  required: [
    'case_id', 'template_id', 'classification', 'summary', 'property_id', 'urgency',
    'evidence', 'missing_information', 'proposed_action', 'draft_reply', 'flags',
    'confidence', 'approval_state'
  ],
  additionalProperties: false
};
...
function cleanCase(caseData) {
  const { gold, ...withoutGold } = caseData || {};
  return withoutGold;
}
...
export async function runShadow({ caseData, template, clientConfig = {}, fetchImpl = fetch }) {
  ...
  return {
    ...enforceShadowPolicy(parsed, template, clientConfig.policy || {}),
    runtime: {
      model,
