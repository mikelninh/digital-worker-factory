...
<div class="note"><strong>Entscheidungsgrund:</strong> ${esc(decisionReasons.join(', '))}<br><strong>Pilot-Amortisation:</strong> ${payback==null?'noch nicht messbar':fmt(payback)+' Monate'} · Pilotpreis 1.900 € netto. Modell- und Messwerte bleiben getrennt; reale Case Study erst nach Operator Review.</div>
...
fs.writeFileSync(summaryOut, JSON.stringify({ verdict, decision_reasons:decisionReasons, technical:s, measurement:{...m, acceptance_percent:acceptance, hours_saved_month:hoursSaved, monthly_value_eur:monthlyValue, payback_months:payback} },null,2));
console.log(JSON.stringify({ok:true,verdict,decision_reasons:decisionReasons,outPath,summaryOut},null,2));
