...
function mockBatch(count = 20) {
  return {
    summary: {
      ...
      completed: count,
      errored: 0,
      gold_checks_passed: count,
      gold_checks_total: count,
      gold_accuracy_percent: 100,
      unsafe_executions: 0,
      ready_for_human_review: count
    },
    rows: Array.from({ length: count }, (_, i) => ({
      case_id: String(i + 1),
      template: 'repair_intake',
      ok: true,
      gold: { passed: 1, total: 1, ok: true },
      result: {
        case_id: String(i + 1),
        ...
        missing_information: [],
        proposed_action: { type: 'internal_review', description: 'Fall prüfen', consequential: false, external: false },
        approval_state: 'shadow_only',
        confidence: 0.95,
        flags: [],
        policy: { execution_allowed: false, human_review_required: true, violations: [] }
      }
    }))
  };
}

test('non-technical assistant standard path closes without founder intervention', async () => {
  const child = spawn(process.execPath, ['packs/hauspilot/operator/ops-console.mjs'], {
    cwd: root,
    env: { ...process.env, HAUSPILOT_OPS_PORT: String(port), OPENAI_API_KEY: '' },
  ...
  try {
    ...
    const unpaid = await post('/api/create', {
      id: id + '-unpaid',
      company: 'Unpaid Demo GmbH',
      template: 'repair_intake',
    ...
    assert.match(unpaid.json.error, /1\.330/);

    const created = await post('/api/create', {
      id,
      company: 'Assistant E2E Demo GmbH',
      template: 'repair_intake',
      data_mode: 'anonymised',
      reviewer_name: 'Reviewer',
    ...
    const intake = await post('/api/intake', {
      id,
      cases_name: 'cases.csv',
      cases_text: makeCasesCsv(20),
      properties_text: 'property_id,address,unit,aliases\nP-1,"Weserstraße 18, Berlin",WE 3,"Weserstr. 18|Weser 18"\n',
      data_mode: 'anonymised',
    ...
    const finalized = await post('/api/finalize', {
      id,
      review_text: JSON.stringify({ reviews }),
      cases_per_month: 220,
      minutes_before: 14,
      minutes_after: 6,
    ...
    assert.ok(fs.existsSync(path.join(deployment, 'pilot-report.local.html')));
    assert.ok(fs.existsSync(path.join(deployment, 'pilot-report.local.summary.json')));

    const close = await post('/api/closeout', {
      id,
...
test('delegated console blocks personal-data mode before processing', async () => {
  const child = spawn(process.execPath, ['packs/hauspilot/operator/ops-console.mjs'], {
    cwd: root,
    env: { ...process.env, HAUSPILOT_OPS_PORT: String(port + 1), OPENAI_API_KEY: '' },
    stdio: ['ignore', 'pipe', 'pipe']
  });
  const localBase = `http://127.0.0.1:${port + 1}`;
  const id = `assistant-privacy-${Date.now()}`;
  const deployment = path.join(root, 'deployments', id);
  const localPost = async (route, body) => {
    const r = await fetch(localBase + route, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
    return { status: r.status, json: await r.json() };
  };
  try {
    for (let i = 0; i < 60; i++) {
      try { if ((await fetch(localBase + '/api/pilots')).ok) break; } catch {}
      await new Promise(r => setTimeout(r, 100));
      if (i === 59) throw new Error('Privacy test console did not start');
    }
    const created = await localPost('/api/create', {
      id,
      company: 'Privacy Exception GmbH',
      template: 'repair_intake',
      data_mode: 'pseudonymised_personal_data',
      reviewer_name: 'Reviewer',
      operator_name: 'Ops',
      deposit_paid: true
    });
    assert.equal(created.status, 200);
    const intake = await localPost('/api/intake', {
      id,
      cases_name: 'cases.csv',
      cases_text: makeCasesCsv(20),
      properties_text: 'property_id,address,unit\nP-1,Weserstraße 18,WE 3\n',
      data_mode: 'pseudonymised_personal_data',
      data_authorised: true,
