...
import { TECHNICAL_DECISIONS, applyAutomaticRemediation, discoverCapabilityGraph, generateAttacks, runAutonomousAttackFixLoop } from './autonomous-security-loop.mjs'

const fixture = JSON.parse(await readFile(new URL('./examples/autonomous-loop-vulnerable-agent.json', import.meta.url), 'utf8'))

test('discovers a capability graph and generates capability-specific attacks', () => {
  ...
  assert.deepEqual(new Set(attacks.map((attack) => attack.kind)), new Set([
    'untrusted_instruction_effect', 'hostile_tool_result', 'approval_bypass', 'parameter_tampering',
    'cross_tenant_effect', 'confused_deputy', 'protected_scope_exfiltration', 'sensitive_egress',
  ]))
})
...
test('autonomously turns real executor escapes into zero-impact regression cases', async () => {
  ...
  assert.equal(report.version, 'autonomous-security-loop/v2')
  assert.equal(report.before.impactEscapes, 8)
  assert.equal(report.before.executorCalls, 8)
...
test('remediation is idempotent after architecture hardening', async () => {
...
test('fails closed when no attack coverage can be generated', async () => {
  const report = await runAutonomousAttackFixLoop({ agent: { id: 'empty-agent' }, actions: [], autonomousSecurity: { securityBoundaryRequired: true, modelMaySelfApproveEffects: false, maxToolCalls: 8 } })
