...
let task;
try {
  project = readJson(".harness/project.json");
} catch (error) {
  fail(`invalid .harness/project.json: ${error.message}`);
}
try {
  task = readJson(".harness/active-task.json");
} catch (error) {
  fail(`invalid .harness/active-task.json: ${error.message}`);
}
...
if (task && project) {
  ...
  if (["A3", "A4"].includes(task.risk_class) && task.approval_required !== true) {
    fail(`${task.risk_class} tasks require human approval`);
  }
  if (!Number.isInteger(task.max_retries) || task.max_retries < 0 || task.max_retries > project.retry_policy.max_retries) {
    fail(`task max_retries must be between 0 and ${project.retry_policy.max_retries}`);
  }
  if (["ready_for_review", "completed"].includes(task.status) && (!Array.isArray(task.evidence) || task.evidence.length === 0)) {
    fail(`${task.status} tasks need evidence`);
  }
  ...
  if (task.status === "completed") {
    if (!task.receipt || !exists(task.receipt)) fail("completed task must reference an existing receipt");
      ...
      try {
        ...
        fail(`invalid completion receipt: ${error.message}`);
      }
    }
  }
}
...
if (exists(receiptDir)) {
  for (const name of fs.readdirSync(path.join(root, receiptDir)).filter((n) => n.endsWith(".json"))) {
    try {
      ...
      if ((receipt.external_actions || []).some((a) => ["A3", "A4"].includes(a.risk_class) && a.approved !== true)) {
        fail(`${name} contains an unapproved consequential external action`);
      }
    } catch (error) {
      fail(`invalid receipt ${name}: ${error.message}`);
    }
  }
}

if (!process.exitCode) {
  console.log(`Harness OK: ${project.project.name} / ${task.task_id} / ${task.status}`);
}
