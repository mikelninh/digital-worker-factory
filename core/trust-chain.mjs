...
export function trustChainDigest(chain) {
  return createHash('sha256').update(JSON.stringify(stable(chain))).digest('hex')
}
...
export function validateTrustChain(chain, { minimumLevel = TRUST_LEVELS.TRACEABLE, approvedBy = null } = {}) {
  ...
  if (!present(chain.provenance?.sourceSystem) || !present(chain.provenance?.sourceUri) || !present(chain.provenance?.acquiredAt)) {
    reasons.push('provenance_required')
  }

  if (!present(chain.authority?.id) || !present(chain.authority?.title) || !present(chain.authority?.version) || !present(chain.authority?.sourceUrl)) {
  ...
  for (const item of chain.evidence ?? []) {
    if (!present(item?.id) || !present(item?.sourceId)) reasons.push('evidence_identity_required')
    if (!present(item?.locator?.kind) || !present(item?.locator?.value)) reasons.push('evidence_exact_locator_required')
    if (!present(item?.excerptHash) || !/^[a-f0-9]{64}$/i.test(item.excerptHash)) reasons.push('evidence_excerpt_hash_required')
    if (present(item?.id)) evidenceIds.add(item.id)
  }
  ...
  for (const id of chain.derivation?.evidenceIds ?? []) {
    if (!evidenceIds.has(id)) reasons.push(`derivation_unknown_evidence:${id}`)
  }
  ...
  const structuralReasons = reasons.filter((reason) => !['authenticity_unverified', 'authority_not_authoritative'].includes(reason))
  if (structuralReasons.length === 0 && chain.authenticity?.status !== 'unverified') level = TRUST_LEVELS.TRACEABLE
...
export function buildEvidenceRef({ id, sourceId, locatorKind, locatorValue, excerpt = '' }) {
  return {
    id,
    sourceId,
    locator: { kind: locatorKind, value: locatorValue },
